...
class GO:
    ...
    def find_liberty_number(self, piece_type):
        '''
        Find liberty of a given stone. If a group of allied stones has no liberty, they all die.
            ...
            for j in range(self.size):
                if board[i][j] == piece_type:
                    ...
                    for member in ally_members:
                        ...
                        for piece in neighbors:
                        # If there is empty space around a piece, it has liberty
                            if board[piece[0]][piece[1]] == 0 and (piece[0],piece[1]) not in liberty:
    ...
    def find_killed_pieces(self, piece_type):
        '''
        Find the died stones that has no liberty in the board for a given piece type.
        ...
        for i in range(len(board)):
            for j in range(len(board)):
                ...
                if board[i][j] == 3-piece_type:
                    # The piece die if it has no liberty
                    if not self.find_liberty(i, j):
                        killed_pieces.append((i,j))
                        cnt += 1
        return cnt

    def find_eyes(self, piece_type):
        '''
        Find the died stones that has no liberty in the board for a given piece type.
